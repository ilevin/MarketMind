...
name 不设数据库 UNIQUE 约束：DuckDB 1.5.5 中父表被 FK 引用后其 UNIQUE 列
不可 UPDATE（误触发 FK 检查，Phase 0 结论），重名校验由 TagService 在写锁内
SELECT 查重保证（单写者模型下无并发窗口）。
"""
...
class Tag(Base):
    """标签；name 全库唯一（TagService 写锁内查重），非空且不超过 50 字符。"""
